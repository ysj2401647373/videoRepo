				...
				<div class="form-group">
					...
					<div class="col-sm-10">
						<select class="form-control" id="inputPassword3" name="speakerId" >
							<c:forEach var="speaker" items="${speaker2}">
							<option value="${speaker.id}" ${speaker.id == speakerId?"selected":""}>${speaker.speakerName}</option>
						   </c:forEach>
						</select>
					</div>
				</div>
				<div class="form-group">
					...
					<div class="col-sm-10">
						<select class="form-control" id="inputPassword3" name="courseId">
								<c:forEach var="subject" items="${subject2}">
							<option value="${subject.id}" ${subject.id ==courseId?"selected":""}>${subject.subjectName}</option>
						   </c:forEach>
						</select>
					</div>
				</div>
